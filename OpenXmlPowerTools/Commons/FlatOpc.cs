...
namespace OpenXmlPowerTools.Commons
{
    public static class FlatOpc
    {
        ...
        private static XProcessingInstruction GetProcessingInstruction(string path)
        {
            ...
            if (Documents.Wordprocessing.IsWordprocessing(fi.Extension))
                return new XProcessingInstruction("mso-application",
                            "progid=\"Word.Document\"");
            if (Presentations.Presentation.IsPresentation(fi.Extension))
                return new XProcessingInstruction("mso-application",
                            "progid=\"PowerPoint.Show\"");
            if (Spreadsheets.Spreadsheet.IsSpreadsheet(fi.Extension))
                return new XProcessingInstruction("mso-application",
                            "progid=\"Excel.Sheet\"");
            return null;
        }
